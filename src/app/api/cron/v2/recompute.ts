...
import {
    calculateAndUpsertPointsBatch,
    fetchAndUpsertAccountData,
    removeUnsubscribedAuthors,
    upsertAuthors,
} from './dataManager';

/**
 * Recomputes a batch of the stalest leaderboard accounts and upserts the results
 * to Supabase. This is pure Node logic (no Next.js) so it can run either:
 *   - inside the Next API route (/api/cron/v2) as a protected manual/fallback trigger, or
 *   - as a standalone scheduled job on the Mac Mini (scripts/recompute-leaderboard.ts).
 *
 * Vercel only ever reads the resulting data via /api/v2/leaderboard.
 */
